...
public final class FrontierSitePiece extends StructurePiece {
    ...
    private static void assembleVault(StructurePiecesBuilder b,FrontierSiteStructure.Kind s,BlockPos c,int v){
        // Surface entry -> stepped descent -> security -> vault core -> archive/refinery side wings.
    ...
    private static void assembleFoundry(StructurePiecesBuilder b,FrontierSiteStructure.Kind s,BlockPos c,int v){
        // Gate -> control -> fabrication -> hangar/loading, with salvage off the main route.
    ...
    private static void assembleQuarantine(StructurePiecesBuilder b,FrontierSiteStructure.Kind s,BlockPos c,int v){
        // Entry -> decon -> security -> observation/control -> containment.
    ...
    private static void assembleRecovery(StructurePiecesBuilder b,FrontierSiteStructure.Kind s,BlockPos c,int v){
        // Entry -> control -> processing/storage -> landmark array.
    ...
    /** Chamfered industrial module with cardinal doors and a permanently clear centre route. */
    ...
    private void entry(WorldGenLevel level,BoundingBox clip,boolean reinforced){
        module(level,clip,6,8,5,reinforced?blackWall():paletteWall(),paletteFloor(),false,p(0,1,3,"holo_sign",Blocks.SEA_LANTERN));
        BlockState beam=mod("decorative.beams",Blocks.POLISHED_DEEPSLATE),stripe=mod("decorative.tritanium_plate_stripe",Blocks.YELLOW_CONCRETE);
        for(int z=-13;z<=-8;z++)for(int x=-4;x<=4;x++)if(Math.abs(x)<=4-Math.abs(z+10)/2){set(level,clip,origin.offset(x,5,z),beam);set(level,clip,origin.offset(x,0,z),x==0?stripe:paletteFloor());}
    ...
    private void vaultCore(WorldGenLevel level,BoundingBox clip){
        ...
        for(int y=1;y<=3;y++)for(int w=-1;w<=1;w++){set(level,clip,origin.offset(w,y,-5),Blocks.AIR.defaultBlockState());set(level,clip,origin.offset(w,y,5),Blocks.AIR.defaultBlockState());}
        set(level,clip,origin.offset(0,1,0),mod("matter_storage_matrix",Blocks.IRON_BLOCK));
        set(level,clip,origin.offset(-7,1,4),mod("tritanium_crate_red",Blocks.BARREL));
        set(level,clip,origin.offset(7,1,4),mod("holographic_status_panel",Blocks.SEA_LANTERN));
    }
    ...
    private void containment(WorldGenLevel level,BoundingBox clip){
        BlockState wall=blackWall(),floor=darkFloor(),beam=mod("decorative.beams",Blocks.POLISHED_DEEPSLATE),glass=mod("industrial_glass",Blocks.TINTED_GLASS),lamp=mod("decorative.tritanium_lamp",Blocks.REDSTONE_LAMP);
        int r=12;
        for(int x=-r;x<=r;x++)for(int z=-r;z<=r;z++){double d=Math.sqrt(x*x+z*z);if(d<=r-1){set(level,clip,origin.offset(x,0,z),floor);for(int y=1;y<=9;y++)set(level,clip,origin.offset(x,y,z),Blocks.AIR.defaultBlockState());}if(d>r-1.4&&d<=r+.2)for(int y=1;y<=9;y++)set(level,clip,origin.offset(x,y,z),y==1||y==9||((x+z)&4)==0?beam:y>=3&&y<=6?glass:wall);if(d<=r)set(level,clip,origin.offset(x,10,z),wall);}
        ...
        for(int a=0;a<360;a+=45){double rad=Math.toRadians(a);int x=(int)Math.round(Math.cos(rad)*7),z=(int)Math.round(Math.sin(rad)*7);set(level,clip,origin.offset(x,2,z),beam);set(level,clip,origin.offset(x,3,z),lamp);}
        set(level,clip,origin.offset(0,1,0),mod("anomaly_containment_unit",Blocks.CRYING_OBSIDIAN));set(level,clip,origin.offset(-6,1,0),mod("gravitational_stabilizer",Blocks.OBSIDIAN));set(level,clip,origin.offset(6,1,0),mod("gravitational_stabilizer",Blocks.OBSIDIAN));set(level,clip,origin.offset(0,1,6),mod("facility_network_controller",Blocks.IRON_BLOCK));
    }
    ...
    private void recoveryArray(WorldGenLevel level,BoundingBox clip){
        BlockState frame=mod("decorative.beams",Blocks.POLISHED_DEEPSLATE),floor=paletteFloor(),glass=mod("industrial_glass",Blocks.TINTED_GLASS),stripe=mod("decorative.tritanium_plate_stripe",Blocks.YELLOW_CONCRETE);
        for(int x=-14;x<=14;x++)for(int z=-14;z<=14;z++){double d=Math.sqrt(x*x+z*z);if(d<=13)set(level,clip,origin.offset(x,0,z),((Math.abs(x)+Math.abs(z))%7==0)?stripe:floor);}
        // Four sloping support arms and a tall central mast create a visible silhouette.
        for(int y=1;y<=18;y++){set(level,clip,origin.offset(0,y,0),frame);int r=Math.max(1,7-y/3);for(int d=-r;d<=r;d++){if(y%3==0){set(level,clip,origin.offset(d,y,0),frame);set(level,clip,origin.offset(0,y,d),frame);}}}
        for(int r=3;r<=10;r+=2)for(int x=-r;x<=r;x++){int z=r-Math.abs(x);set(level,clip,origin.offset(x,10+r/2,z),glass);set(level,clip,origin.offset(x,10+r/2,-z),glass);}
        set(level,clip,origin.offset(0,1,-6),mod("quantum_power_relay",Blocks.RESPAWN_ANCHOR));set(level,clip,origin.offset(6,1,5),mod("android_spawner",Blocks.IRON_BLOCK));
    }
