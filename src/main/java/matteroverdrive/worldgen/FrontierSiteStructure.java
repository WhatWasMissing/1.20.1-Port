...
/** New Frontier starts use exploration-native pieces; old serializers remain registered for existing saves. */
public final class FrontierSiteStructure extends Structure {
    private static final Logger LOGGER = LogUtils.getLogger();
    public enum Kind { DEEP_MATTER_VAULT, AUTONOMOUS_DRONE_FOUNDRY, ANOMALY_QUARANTINE_SITE, ORBITAL_RECOVERY_ARRAY }
    private final Kind kind;
    public FrontierSiteStructure(StructureSettings settings, Kind kind){super(settings);this.kind=kind;}
    public static Codec<FrontierSiteStructure> codec(Kind kind){return simpleCodec(settings -> new FrontierSiteStructure(settings,kind));}
