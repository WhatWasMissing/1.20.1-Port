...
public final class LegacyStructureFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private static boolean crashedShip(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        for (int z = -8; z <= 5; z += 3) {
            set(level, base.offset(-4, 1, z), stripe);
            set(level, base.offset(4, 1, z), stripe);
        }
        ...
        for (int i = 0; i < defenders; i++) spawnAndroid(level, base.offset((i * 3) - 1, 1, -2 + i * 4), random);
        return true;
    }
    ...
    private static boolean cargoShip(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        for (int z = -20; z <= 20; z++) {
            ...
            for (int x = -half; x <= half; x++) {
                ...
                if (Math.abs(x) == half) {
                    ...
                    if (z % 2 == 0) set(level, base.offset(x, 2, z), hull);
                }
            }
        }
        for (int z = -13; z <= 13; z++) {
            set(level, base.offset(-3, 3, z), hull);
            set(level, base.offset(3, 3, z), hull);
            if (z % 2 == 0) {
                set(level, base.offset(-2, 3, z), block("industrial_glass"));
                set(level, base.offset(2, 3, z), block("industrial_glass"));
            }
        }
        ...
        for (int i = 0; i < defenders; i++) spawnAndroid(level, base.offset((i % 2 == 0 ? -2 : 2), 1, -9 + i * 6), random);
        if (random.nextInt(3) == 0) spawnMob(level, base.offset(0, 2, 6), ModEntities.DRONE.get(), random);
        return true;
    }
    ...
    private static boolean underwaterBase(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        spawnMob(level, c.offset(-3, 1, 0), ModEntities.DRONE.get(), random);
        if (random.nextBoolean()) spawnMob(level, c.offset(0, 1, 4), ModEntities.RANGED_ROGUE_ANDROID.get(), random);
        return true;
    }
    ...
    private static boolean madScientistHouse(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        for (int x = -5; x <= 5; x++) for (int z = -5; z <= 5; z++) {
            ...
            if (Math.abs(x) == 5 || Math.abs(z) == 5) for (int y = 1; y <= 4; y++) {
                boolean window = y >= 2 && y <= 3 && ((Math.abs(x) == 5 && Math.abs(z) <= 2) || (Math.abs(z) == 5 && Math.abs(x) <= 2));
                set(level, origin.offset(x, y, z), window ? glass : plate);
            }
            set(level, origin.offset(x, 5, z), plate);
        }
        for (int y = 1; y <= 4; y++) {
            set(level, origin.offset(-4, y, -4), beam);
            set(level, origin.offset(4, y, -4), beam);
            set(level, origin.offset(-4, y, 4), beam);
            set(level, origin.offset(4, y, 4), beam);
        }
        ...
        if (random.nextBoolean()) spawnMob(level, origin.offset(3, 1, -3), failed, random);
        return true;
    }
    ...
    private static boolean androidHouse(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        BlockPos[] positions = {origin.offset(-4, 1, 0), origin.offset(4, 1, 0), origin.offset(-2, 1, -5), origin.offset(2, 1, -5)};
        for (int i = 0; i < defenders; i++) spawnAndroid(level, positions[i], random);
        if (random.nextInt(4) == 0) spawnMob(level, origin.offset(0, 2, 2), ModEntities.DRONE.get(), random);
        return true;
    }
    ...
    private static boolean sandPit(WorldGenLevel level, BlockPos origin, RandomSource random) {
        ...
        spawnAndroid(level, floor.offset(3, 1, 2), random);
        if (random.nextInt(5) == 0) spawnMob(level, floor.offset(-3, 2, -1), ModEntities.DRONE.get(), random);
        return true;
    }
