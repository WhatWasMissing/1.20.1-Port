...
public class TransporterBlockEntity extends BlockEntity implements MenuProvider {
    ...
    @Override protected void saveAdditional(CompoundTag t){super.saveAdditional(t);t.put("Items",items.serializeNBT());t.put("Upgrades",upgrades.serializeNBT());t.putInt("Energy",energy.getEnergyStored());t.putInt("Progress",progress);t.putInt("Cooldown",cooldown);}
    @Override public void load(CompoundTag t){super.load(t);if(t.contains("Items"))items.deserializeNBT(t.getCompound("Items"));if(t.contains("Upgrades"))upgrades.deserializeNBT(t.getCompound("Upgrades"));upgradesChanged();energy.setEnergyStored(t.getInt("Energy"));progress=t.getInt("Progress");cooldown=t.getInt("Cooldown");}
