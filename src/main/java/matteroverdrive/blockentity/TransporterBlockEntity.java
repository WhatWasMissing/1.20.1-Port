...
public class TransporterBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, TransporterBlockEntity t){t.energy.beginUsageTick(level.getGameTime());t.charge();t.tickTransport();}
    ...
    private void tickTransport(){if(cooldown>0){cooldown--;running=false;return;}if(!targetValid()||energy.getEnergyStored()<energyCost()||entities().isEmpty()){progress=0;running=false;return;}running=true;if(++progress>=cycle()){BlockPos target=TransportFlashDriveItem.getTarget(items.getStackInSlot(DRIVE_SLOT));for(Entity e:entities())e.teleportTo(target.getX()+0.5,target.getY()+1,target.getZ()+0.5);energy.consumeEnergy(energyCost(),level.getGameTime());progress=0;cooldown=delay();setChanged();}}
