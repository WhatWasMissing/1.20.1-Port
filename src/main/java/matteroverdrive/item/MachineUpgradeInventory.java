...
public class MachineUpgradeInventory extends ItemStackHandler {
    ...
    public boolean isItemValid(int slot, ItemStack stack) {
        return stack.getItem() instanceof MachineUpgradeItem item && supported.test(item.getUpgrade());
    }
    ...
    public double getMultiplier(ToDoubleFunction<MachineUpgradeItem.Upgrade> effect) {
        ...
        for (int slot = 0; slot < getSlots(); slot++) {
            ...
            if (stack.getItem() instanceof MachineUpgradeItem item && supported.test(item.getUpgrade())) {
                multiplier *= effect.applyAsDouble(item.getUpgrade());
            }
        }
        return multiplier;
    }
