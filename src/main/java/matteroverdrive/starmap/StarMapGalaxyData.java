...
public final class StarMapGalaxyData extends SavedData {
    ...
    public PlanetState planet(int quadrant, int star, int planet) { return planets.computeIfAbsent(key(quadrant, star, planet), ignored -> new PlanetState()); }
    public boolean ensureHomeworld(int quadrant,int star,int planet,UUID owner){PlanetState state=planet(quadrant,star,planet);if(state.owner!=null&&!state.owner.equals(owner))return false;boolean changed=state.owner==null||!state.base||!state.shipFactory;state.owner=owner;state.base=true;state.shipFactory=true;if(changed)setDirty();return true;}
    public boolean colonize(int quadrant,int star,int planet,UUID owner){PlanetState state=planet(quadrant,star,planet);if(state.owner!=null||state.base)return false;state.owner=owner;state.base=true;setDirty();return true;}
    ...
    public boolean addArrivingShip(int q,int s,int p,UUID owner,int shipType){PlanetState state=planet(q,s,p);if(!state.isOwner(owner)||!state.base||state.shipCount()>=state.fleetCapacity())return false;if(shipType==StarMapShipTravelData.SHIP_SCOUT)state.scoutShips++;else if(shipType==StarMapShipTravelData.SHIP_COLONIZER)state.colonizerShips++;else return false;setDirty();return true;}
    ...
    public static final class PlanetState {
        ...
        public int fleetCapacity(){return 2+hangars*2;}public int buildingCapacity(){return (base?2:0)+residential*4;}public int buildingCount(){return (base?1:0)+(shipFactory?1:0)+hangars+matterExtractors+powerGenerators+residential;}
        public int energyProduction(){return powerGenerators*8-matterExtractors*6-residential*4;}public int matterProduction(){return matterExtractors*10-powerGenerators*2-residential*2;}public int population(){return residential*10000;}
