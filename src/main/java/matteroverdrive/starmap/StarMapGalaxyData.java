...
public final class StarMapGalaxyData extends SavedData {
    ...
    public boolean ensureHomeworld(int quadrant, int star, int planet, UUID owner) {
        ...
        state.owner = owner; state.base = true; state.shipFactory = true;
        if (changed) setDirty(); return true;
    }

    public boolean colonize(int quadrant, int star, int planet, UUID owner) {
        PlanetState state = planet(quadrant, star, planet);
        if (state.owner != null || state.base) return false;
        state.owner = owner; state.base = true; setDirty(); return true;
    }
    ...
    /** Restores the four per-planet construction inventory slots present in the 1.7 Planet implementation. */
    public boolean startBuild(int q,int s,int p,UUID owner,int action,long endTime){PlanetState state=planet(q,s,p);if(!state.isOwner(owner)||!state.base||state.builds.size()>=MAX_BUILD_SLOTS)return false;state.builds.add(new BuildProject(action,Math.max(1,endTime)));setDirty();return true;}
    /** Save bridge for the immediately preceding console-local single construction queue. */
    ...
    public List<BuildCompletion> collectCompletedBuilds(UUID owner,long now){List<BuildCompletion> out=new ArrayList<>();boolean changed=false;for(var entry:planets.entrySet()){PlanetState state=entry.getValue();if(!state.isOwner(owner)||state.builds.isEmpty())continue;int[] pos=parseKey(entry.getKey());Iterator<BuildProject> it=state.builds.iterator();while(it.hasNext()){BuildProject build=it.next();if(now<build.endTime)continue;out.add(new BuildCompletion(pos[0],pos[1],pos[2],build.action));it.remove();changed=true;}}if(changed)setDirty();return out;}
    ...
    @Override public CompoundTag save(CompoundTag tag){ListTag list=new ListTag();for(var entry:planets.entrySet()){PlanetState state=entry.getValue();CompoundTag e=new CompoundTag();e.putString("Key",entry.getKey());if(state.owner!=null)e.putUUID("Owner",state.owner);e.putBoolean("Base",state.base);e.putBoolean("ShipFactory",state.shipFactory);e.putInt("Hangars",state.hangars);e.putInt("MatterExtractors",state.matterExtractors);e.putInt("PowerGenerators",state.powerGenerators);e.putInt("Residential",state.residential);e.putInt("ScoutShips",state.scoutShips);e.putInt("ColonizerShips",state.colonizerShips);ListTag builds=new ListTag();for(BuildProject build:state.builds){CompoundTag b=new CompoundTag();b.putInt("Action",build.action);b.putLong("End",build.endTime);builds.add(b);}e.put("Builds",builds);list.add(e);}tag.put("Planets",list);return tag;}

    private static StarMapGalaxyData load(CompoundTag tag){StarMapGalaxyData data=new StarMapGalaxyData();ListTag list=tag.getList("Planets",10);for(int i=0;i<list.size();i++){CompoundTag e=list.getCompound(i);String key=e.getString("Key");if(key.isEmpty())continue;PlanetState state=new PlanetState();state.owner=e.hasUUID("Owner")?e.getUUID("Owner"):null;state.base=e.getBoolean("Base");state.shipFactory=e.getBoolean("ShipFactory");state.hangars=clamp(e.getInt("Hangars"),0,8);state.matterExtractors=clamp(e.getInt("MatterExtractors"),0,16);state.powerGenerators=clamp(e.getInt("PowerGenerators"),0,16);state.residential=clamp(e.getInt("Residential"),0,16);state.scoutShips=Math.max(0,e.getInt("ScoutShips"));state.colonizerShips=Math.max(0,e.getInt("ColonizerShips"));ListTag builds=e.getList("Builds",10);for(int j=0;j<builds.size()&&state.builds.size()<MAX_BUILD_SLOTS;j++){CompoundTag b=builds.getCompound(j);int action=b.getInt("Action");if(action>0)state.builds.add(new BuildProject(action,Math.max(1,b.getLong("End"))));}data.planets.put(key,state);}return data;}
    private static int clamp(int value,int min,int max){return Math.max(min,Math.min(max,value));}

    public static final class PlanetState {
        @Nullable private UUID owner;private boolean base;private boolean shipFactory;private int hangars,matterExtractors,powerGenerators,residential,scoutShips,colonizerShips;private final List<BuildProject> builds=new ArrayList<>();
        public PlanetState(){}
        ...
        /** Legacy Matter Extractor +10 matter/-6 energy; Power Generator +8 energy/-2 matter; Residential -4 energy/-2 matter. */ public int energyProduction(){return powerGenerators*8-matterExtractors*6-residential*4;}public int matterProduction(){return matterExtractors*10-powerGenerators*2-residential*2;}
        /** Legacy Residential adds 10,000 population. */ public int population(){return residential*10000;}
        /** Legacy Residential happiness: +0.5/-0.4 for power and +0.5/-0.6 for matter, based on colony totals. Returned as hundredths for menu sync. */ public int happinessHundredths(){if(residential<=0)return 0;float per=(energyProduction()>=0?.5f:-.4f)+(matterProduction()>=0?.5f:-.6f);return Math.round(per*residential*100f);}
    }
