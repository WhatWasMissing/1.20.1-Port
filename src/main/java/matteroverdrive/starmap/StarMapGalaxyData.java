...
/** Server-global persistent ownership, building and production state for deterministic Star Map planets. */
public final class StarMapGalaxyData extends SavedData {
    ...
    @Override public CompoundTag save(CompoundTag tag){
        ...
        for(var entry:planets.entrySet()){
            ...
            e.putInt("Hangars",state.hangars);e.putInt("MatterExtractors",state.matterExtractors);e.putInt("PowerGenerators",state.powerGenerators);e.putInt("Residential",state.residential);list.add(e);
        }
    ...
    private static StarMapGalaxyData load(CompoundTag tag){
        ...
        for(int i=0;i<list.size();i++){
            ...
            state.hangars=clamp(e.getInt("Hangars"),0,8);state.matterExtractors=clamp(e.getInt("MatterExtractors"),0,16);state.powerGenerators=clamp(e.getInt("PowerGenerators"),0,16);state.residential=clamp(e.getInt("Residential"),0,16);data.planets.put(key,state);
        }
        return data;
    }
    ...
    public static final class PlanetState {
        @Nullable private UUID owner;private boolean base;private boolean shipFactory;private int hangars,matterExtractors,powerGenerators,residential;
        public PlanetState(){}
