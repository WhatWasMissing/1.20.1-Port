...
public final class ResearchCampaignQuestFlow {
    ...
    public static boolean handle(ServerPlayer player) {
        ...
        if (data.getBoolean(DONE)) return false;
        int index = normalize(player, data);
        ItemStack active = find(player, index);
        if (!active.isEmpty()) {
            if (!ContractItem.complete(active)) { showProgress(player, active, index); return true; }
            redeem(player, active, index, data); return true;
        }
        if (index >= ResearchCampaignContracts.QUEST_IDS.length) { completeCampaign(player, data); return true; }
        issue(player, index, data); return true;
    }

    public static boolean complete(ServerPlayer player) { return data(player).getBoolean(DONE); }
    public static void showEndgame(ServerPlayer player) {
        ModNetwork.openDialogue(player, "Dr. Voss - Research Directorate", "Independent Endgame Research", List.of(
                ...
                "Special projects such as Cocktail of Ascension remain available from the appropriate scientist."));
    }
    public static String status(ServerPlayer player) {
        ...
        return active.isEmpty() ? stage + " - next: " + title(ResearchCampaignContracts.QUEST_IDS[i]) : stage + " - " + ContractItem.title(active) + " (" + ContractItem.progress(active) + "/" + ContractItem.goal(active) + ")";
    }

    private static void issue(ServerPlayer player, int index, CompoundTag data) {
        String id = ResearchCampaignContracts.QUEST_IDS[index]; ItemStack quest = ResearchCampaignContracts.create(id, player.getRandom()); if (quest.isEmpty()) return;
        ResearchProgression.Stage stage = ResearchCampaignContracts.stageForIndex(index); ResearchProgression.unlock(player, stage); give(player, quest); save(player, data);
        player.level().playSound(null, player.blockPosition(), ModSounds.get("gui.quest_started").get(), SoundSource.NEUTRAL, 1.0F, 1.0F);
        ModNetwork.openDialogue(player, scientist(stage), ContractItem.title(quest), List.of(stageIntro(stage), briefing(id), objective(quest), "Research stage: " + stage.title, "Complete the field objective and return with the contract."));
    }
    private static void showProgress(ServerPlayer player, ItemStack active, int index) {
        ...
        ModNetwork.openDialogue(player, scientist(stage), ContractItem.title(active), List.of(progressLine(ContractItem.contractId(active)), objective(active), "Progress: " + ContractItem.progress(active) + " / " + ContractItem.goal(active), "Research stage: " + stage.title));
    }
    private static void redeem(ServerPlayer player, ItemStack completed, int index, CompoundTag data) {
        String id = ContractItem.contractId(completed); for (ItemStack reward : ContractItem.rewardItems(completed)) give(player, reward.copy()); if (ContractItem.xp(completed) > 0) player.giveExperiencePoints(ContractItem.xp(completed));
        remove(player, completed); cleanup(player, index); int next = Math.max(index + 1, data.getInt(INDEX)); data.putInt(INDEX, next);
        ResearchProgression.Stage oldStage = ResearchCampaignContracts.stageForIndex(index); ResearchProgression.Stage nextStage = next < ResearchCampaignContracts.QUEST_IDS.length ? ResearchCampaignContracts.stageForIndex(next) : ResearchProgression.Stage.ANOMALY_ENGINEERING;
        if (nextStage.ordinal() > oldStage.ordinal()) ResearchProgression.unlock(player, nextStage); if (next >= ResearchCampaignContracts.QUEST_IDS.length) data.putBoolean(DONE, true); save(player, data);
        player.level().playSound(null, player.blockPosition(), ModSounds.get("gui.quest_complete").get(), SoundSource.NEUTRAL, 1.0F, 1.0F);
        if (next >= ResearchCampaignContracts.QUEST_IDS.length) { completeCampaign(player, data); return; }
        String nextId = ResearchCampaignContracts.QUEST_IDS[next];
        ModNetwork.openDialogue(player, scientist(oldStage), ContractItem.title(completed) + " Complete", List.of(completion(id), nextStage.ordinal() > oldStage.ordinal() ? "NEW RESEARCH STAGE: " + nextStage.title : "Research continues: " + oldStage.title, "Reward package delivered.", "Next assignment: " + title(nextId) + "."));
    }
    private static void completeCampaign(ServerPlayer player, CompoundTag data) {
        data.putBoolean(DONE, true); data.putInt(INDEX, ResearchCampaignContracts.QUEST_IDS.length); save(player, data); ResearchProgression.unlock(player, ResearchProgression.Stage.ANOMALY_ENGINEERING);
        ModNetwork.openDialogue(player, "Dr. Voss - Research Directorate", "Anomaly Engineering Complete", List.of("You began as a research subject and ended as the person I would call when spacetime starts making noises.", "Matter Technology, Automation, Advanced Power, Fusion Research and Anomaly Engineering are now complete.", "No further clearance exists. Any larger anomaly you create is officially your responsibility.", "Independent endgame research unlocked. Special projects remain available."));
    }

    private static int normalize(ServerPlayer player, CompoundTag data) { int stored=Math.max(0,data.getInt(INDEX)),furthest=-1; for(ItemStack stack:player.getInventory().items){if(!(stack.getItem() instanceof ContractItem))continue;int i=indexOf(ContractItem.contractId(stack));if(i>=0)furthest=Math.max(furthest,i);}int normalized=Math.min(Math.max(stored,furthest),ResearchCampaignContracts.QUEST_IDS.length);if(normalized!=stored){data.putInt(INDEX,normalized);save(player,data);}return normalized; }
    private static ItemStack find(ServerPlayer player,int wanted){ItemStack fallback=ItemStack.EMPTY;int furthest=-1;for(ItemStack stack:player.getInventory().items){if(!(stack.getItem() instanceof ContractItem))continue;int i=indexOf(ContractItem.contractId(stack));if(i<0)continue;if(i==wanted)return stack;if(i>furthest){fallback=stack;furthest=i;}}return fallback;}
    private static int indexOf(String id){for(int i=0;i<ResearchCampaignContracts.QUEST_IDS.length;i++)if(ResearchCampaignContracts.QUEST_IDS[i].equals(id))return i;return -1;} private static void remove(ServerPlayer p,ItemStack target){for(int i=0;i<p.getInventory().getContainerSize();i++)if(p.getInventory().getItem(i)==target){p.getInventory().setItem(i,ItemStack.EMPTY);return;}} private static void cleanup(ServerPlayer p,int through){for(int i=0;i<p.getInventory().getContainerSize();i++){ItemStack s=p.getInventory().getItem(i);if(s.getItem() instanceof ContractItem){int q=indexOf(ContractItem.contractId(s));if(q>=0&&q<=through)p.getInventory().setItem(i,ItemStack.EMPTY);}}}
    private static CompoundTag data(ServerPlayer p){return p.getPersistentData().getCompound(Player.PERSISTED_NBT_TAG);} private static void save(ServerPlayer p,CompoundTag d){p.getPersistentData().put(Player.PERSISTED_NBT_TAG,d);p.getInventory().setChanged();} private static void give(ServerPlayer p,ItemStack s){if(!p.getInventory().add(s))p.drop(s,false);} private static String objective(ItemStack s){String stage=ContractStageSupport.stageLabel(s);return stage.isBlank()?ContractItem.objectiveText(s):stage+": "+ContractItem.objectiveText(s);} private static String title(String id){ItemStack q=ResearchCampaignContracts.create(id,net.minecraft.util.RandomSource.create(0));return q.isEmpty()?id:ContractItem.title(q);}
