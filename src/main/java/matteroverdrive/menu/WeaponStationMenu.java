...
public class WeaponStationMenu extends AbstractContainerMenu {
    ...
    @Override public ItemStack quickMoveStack(Player player, int index) { Slot slot=slots.get(index); if(!slot.hasItem()) return ItemStack.EMPTY; ItemStack stack=slot.getItem(); ItemStack copy=stack.copy(); if(index<7 ? !moveItemStackTo(stack,7, player.getInventory().getContainerSize()+7,true) : !moveItemStackTo(stack,0,7,false)) return ItemStack.EMPTY; slot.setChanged(); return copy; }
}
