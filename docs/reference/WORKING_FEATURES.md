...
The native Destiny set contains 49 registered energy weapons. The original 14 use the existing imported Destiny geometry, animations and textures; 35 additional guns from the supplied `Destiny_GunPack_v1.5.2.zip` now have native vanilla-rendered geometry, source UV textures, available source animations, individual core fire/third-person fire recordings and available draw/reload recordings. All Destiny weapons use the Matter Overdrive Weapon Station, battery modules, capacitor storage, FE payment and existing barrel/sights/colour/utility module effects. The loader accepts both the older converted animation names and the GunPack's native `static_idle`/`shoot`/reload naming, and handles both compact and per-face Bedrock UV forms. Thirteen new models had no source animation file and therefore use an explicit static idle fallback; Stolen Will uses the shared Universal Remote fire samples specified by the source display definition. The five original profiles absent from the GunPack retain their existing source recordings rather than receiving mismatched substitutions. The Vex Mythoclast remains a separate Matter Overdrive exotic and uses its documented fallback sound events because the supplied Destiny packs contain no Mythoclast recording.
